        ...
        <form id="form1" name="form1" method="post" action="">
            ...
            <center>
                <div class="col-lg-6 grid-margin grid-margin-lg-0 stretch-card">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card-title">Sales Per Product -Pie Chart</h4>
                            <h4>
                                <?php if (isset($_POST['btnsave'])) {
                                    echo " between " . $_POST["txt_f"] . " and " . $_POST["txt_t"];
                                } ?>
                            </h4>
            ...
            if (isset($_POST["btnsave"])) {
                ...
                $sel = "select * from tbl_product where Seller_id=" . $sid . "";
                $row = $conn->query($sel);
                while ($data = $row->fetch_assoc()) {
                    $xValues[] = $data["prod_name"];
                    $sel1 = "select IFNULL(sum(cart_quantity), 0) as id from tbl_cart c inner join tbl_order o on o.order_id=c.order_id inner join tbl_product p on c.product_id=p.product_id where ( cart_status in (1,3,4) and p.product_id = '" . $data["product_id"] . "'and o.order_date between '" . $_POST["txt_f"] . "' and '" . $_POST["txt_t"] . "')";
                                ...
                                <div class="table-responsive">
                                    <table class="table table-hover">
                                        ...
                                        <tbody>
                                            ...
                                            $sel = " select * from tbl_cart c inner join tbl_order o on o.order_id=c.order_id inner join tbl_customer cu on o.customer_id=cu.customer_id inner join tbl_product p on c.product_id=p.product_id where ( cart_status in (1,3,4) and seller_id=" . $sid . " and o.order_date between '" . $_POST["txt_f"] . "' and '" . $_POST["txt_t"] . "')";
                                            $row = $conn->query($sel);
                                            $i = 0;
                                            while ($data = $row->fetch_assoc()) {
                                                $i++;
